...
import {Alert, Text, View, TouchableOpacity, StyleSheet, TextInput, ScrollView} from 'react-native';
...
import {ENDPOINTS, FORM_STATUS_OBJ} from '../Constants';
...
const FormReview = ({route, navigation}) => {
    ...
    return (
      <View style={styles.FormDetailsTileContainer}>
        <View
          ...
          <Text style={{color: '#282C50', fontSize: 20, fontWeight: '500', paddingHorizontal: 10}}>
            {farm}
          </Text>
        ...
        <View
          ...
          <View>
            ...
            <Text>
              Date Created: {InnerText(convertToJSCompatibleFormat(dateCreated).toDateString())}
            </Text>
          </View>
          <View>
            ...
            <Text>
              Date Submitted:{' '}
              {typeof dateSubmitted == 'undefined'
                ? InnerText('No Date Found')
                : InnerText(convertToJSCompatibleFormat(dateSubmitted).toDateString())}
            </Text>
          </View>
        </View>
      </View>
  ...
  const InnerText = text => {
    return (
      <Text style={{fontWeight: '600', fontStyle: 'italic', color: 'black', fontSize: 16}}>
        {text}
      </Text>
    );
  };
    ...
    <LinearGradient
      ...
      {reject && (
        <RejectMenu
          ...
          id={sturctedForm.header['Form Id']}
          setReject={setReject}
        />
      )}
      {!reject && (
        <View
          ...
          <TouchableOpacity
            ...
            onPress={() => approveButton(sturctedForm.header['Form Id'], auth, navigation)}>
            <WhiteTick size={50} />
            <Text style={{color: 'white', fontWeight: '600', fontSize: 20, marginLeft: 20}}>
              APPROVE
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.rejectButton} onPress={() => rejectButton(setReject)}>
            <WhiteX size={40} />
            <Text style={{color: 'white', fontWeight: '600', fontSize: 20, marginLeft: 20}}>
              REJECT
            </Text>
    ...
    <View style={{backgroundColor: '#fbe9ea', borderRadius: 10}}>
      ...
      <View
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            submitRejection(id, auth, reasons, navigation);
          }}>
...
const approveButton = (id, auth, nav) => {
  Alert.alert(
    ...
    [{text: 'Yes, Approve', onPress: () => approveForm(id, auth, nav)}, {text: 'No, Cancel'}],
  );
};

const rejectButton = e => {
  ShowAlert('Alert', "Are you sure you'd like to REJECT this form?", [
  ...
  executeApiQuery(
    ...
    .then(function (response) {
      if (response.status == 200) {
        ShowAlert(`Success`, 'Form rejection submitted successfully' || 'NO Message Returned', [
        ...
        ShowAlert(
          ...
          `${response.response.data.message || 'NO Error Code Found'}`,
        );
      }
    })
    .catch(err => {
...
const approveForm = async (formId, auth, nav) => {
  ...
  executeApiQuery(
    ...
    token,
    'patch',
    {statusId: 2, approverComments: ['Approved'], approvedBy: name},
    undefined,
  )
    .then(function (response) {
      if (response.status == 200) {
        ShowAlert(`Success`, 'Form approval submitted successfully' || 'NO Message Returned', [
          {text: 'Cancel', style: 'cancel'},
          {text: 'OK', onPress: () => nav.goBack()},
        ]);
      } else {
        ShowAlert(
          `Failed - ${response.response.status}`,
          `${response.response.data.message || 'NO Error Code Returned'}`,
        );
      }
    })
    .catch(err => {
