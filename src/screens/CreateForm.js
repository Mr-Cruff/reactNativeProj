...
import {convertToCSharpCompatibleFormat, nth, ShowAlert} from '../services/Helpers';
import {MONTH} from '../Constants';
import {saveForm} from '../services/FormManagement';
...
const CreateForm = ({route, navigation}) => {
  ...
  const HouseSelect = () => {
    return (
      <View>
        <Text style={{marginTop: 10, marginBottom: 10, color: '#9EADD3', textAlign: 'center'}}>
  ...
  //     } catch (error) {
  //       ShowAlert('Error!', 'Unable to save FORM');
  //     }
  //   }
  // };
  ...
  const confirmNewForm = async () => {
    ...
    if (farmSelected != 'none' && houseSelected != 'none' && farmHouse != null) {
      if ((await doesFormExist(newForm['Form Id'])) === true) {
        ShowAlert(
          'Error!',
          'This form has already been created. Go to `Edit Forms` to make changes to this form, otherwise the form has been submitted and cannot be re-created or edited.',
        );
      } else {
        saveForm(newForm);
        ShowAlert(`Notification`, `Would you like to edit this form now?`, [
          {
            text: 'No, Close',
          ...
          },
          {
            text: 'Yes, Edit form',
            onPress: () =>
              navigation.navigate('Edit Form', {formSelected: newForm, farmSelected: farmHouse}),
          },
        ]);
      }
    }
  ...
  const createFormBody = () => {
    ...
    return {
      'Form Id': formIdCalc(Farm, House, date),
      Farm: Farm.name,
      House: House.name,
      FarmNo: Farm.farmNo,
      'Created By': name,
      'Date Created': convertToCSharpCompatibleFormat(date),
      Status: 'Incomplete',
    };
  };
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={{backgroundColor: 'beige'}}>
          <View>
            ...
            <Text style={{paddingLeft: 10, marginTop: 10}}>
              Were 'Eggs Delivered' from {farms[farmSelected].name} on {MONTH[date.getMonth()]}{' '}
              {date.getDate()}
              {nth(date.getDate())}?
            </Text>
          </View>
          <View style={{flexDirection: 'row', justifyContent: 'flex-end'}}>
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate('Egg Delivery', {farmSelected: farms[farmSelected]})
              }>
      ...
      {farmSelected != 'none' &&
        farms[farmSelected].type.toLowerCase() != 'grow' &&
        houseSelected !== 'none' && (
          <View style={{backgroundColor: 'beige', marginTop: 40}}>
            <View>
              ...
              <Text style={{paddingLeft: 10, marginTop: 10}}>
                ...
                <Text style={{fontWeight: 'bold'}}>Birds Capitalized</Text> transaction for{' '}
                {farms[farmSelected].name}, {farms[farmSelected]?.houses[houseSelected].name}, on{' '}
            ...
            <View style={{flexDirection: 'row', justifyContent: 'flex-end'}}>
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate('Birds Capitalized', {
                    farmSelected: farms[farmSelected],
                    house: farms[farmSelected]?.houses[houseSelected],
                  })
                }>
      ...
      <TouchableOpacity
        disabled={farmSelected == 'none' || houseSelected == 'none'}
        style={{maxWidth: 250, alignSelf: 'flex-end', width: 200, marginTop: '5%'}}
        onPress={() => {
          setLoading(true);
          confirmNewForm();
        }}>
        <Text
          style={{
            textAlign: 'center',
            backgroundColor: farmSelected == 'none' || houseSelected == 'none' ? 'grey' : '#282C50',
            borderRadius: 5,
            padding: 10,
            fontSize: 17,
            color: 'white',
