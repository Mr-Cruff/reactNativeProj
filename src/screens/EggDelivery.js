...
import {executeApiQuery, ShowAlert} from '../services/Helpers';
...
const EggDelivery = ({route, navigation}) => {
  ...
  useEffect(() => {
    getEggData({farmId: farmSelected.id, date}).then(e => {
      setTileVal({...farmSelected, ...e});
      setLoading(false);
    });
  }, []);
  ...
  const getEggData = async ({farmId, date = new Date().toJSON()}) => {
    const url = ENDPOINTS.GetEggs;
    const eggData = await executeApiQuery(url, token, 'GET', undefined, {
      farmID: farmId,
      dateCaptured: date,
    }).catch(err => {
      console.log('err');
  ...
  const validateSubmission = () => {
    if (!hed || !sed || hed == '' || sed == '' || (hed == 0 && sed == 0)) {
      ShowAlert(`Error`, `Please enter a VALID value for Egg Delivery Records`, [
        {text: 'OK', onPress: () => setLoading(false)},
      ]);
    } else if (hed > tileVal.weekToDate?.eggsOnHand) {
      ShowAlert(
        `Warning`,
        `Hatching Eggs Delivery value is larger than Hatching Eggs on Hand,\nWould you still like to continue submission?`,
        [
      ...
      ShowAlert(
        `Warning`,
        `Salable Reject Eggs Delivery value is larger than Reject and Double-yolked Eggs collected,\nWould you still like to continue submission?`,
        [
          {text: 'NO', onPress: () => setLoading(false)},
          {text: 'YES', onPress: () => submitEggDelivery(farmSelected.id, date)},
        ],
      );
    } else submitEggDelivery(farmSelected.id, date);
  };

  const submitEggDelivery = async (farmId, date = new Date()) => {
    const eggsDelivered = hed;
    const salableEggsDelivered = sed;
    const url = ENDPOINTS.EggDelivery;

    const eggData = await executeApiQuery(
      url,
      token,
      'POST',
      {
        Farm: farmId,
        'Hatching Eggs Delivered': eggsDelivered,
        'Salable Eggs Delivered': salableEggsDelivered,
        'Date Captured': date,
      },
      undefined,
    ).catch(err => {
    ...
    if (eggData.status) {
      ...
      if (eggData.status == 200) {
        ShowAlert(`Success`, `Egg delivery record was submitted sucessfully.`, [
          {
            text: 'Cancel',
        ...
        ShowAlert(`Failed`, `Egg delivery record submission failed.\nError: ${eggData.status}.`, [
          {
            text: 'Cancel',
            onPress: () => console.log('Cancel Pressed'),
            style: 'cancel',
  ...
  const handleChange = e => {
    const regex = /^[0-9\b]+$/;
    if (e === '' || regex.test(e)) setHed(e);
  };

  const handleChange1 = e => {
    const regex = /^[0-9\b]+$/;
    if (e === '' || regex.test(e)) setSed(e);
  };
  ...
  const RenderTile = () => {
    return (
      <View style={{backgroundColor: '#f9f4f8'}}>
        ...
        <View
          style={{marginHorizontal: 20, flexDirection: 'column', justifyContent: 'space-evenly'}}>
          <View style={{marginBottom: 10}}>
            <View style={{marginHorizontal: -20, backgroundColor: '#164d59', alignItems: 'center'}}>
              <Text style={{fontSize: 20, fontWeight: 'medium', color: '#bfe7ef'}}>
                WEEK TO DATE
              </Text>
            </View>
            <RenderLabelValue
              label={'Eggs Brought Forward'}
              value={tileVal.weekToDate?.eggBroughtForward}
            />
            <RenderLabelValue
              label={'Eggs Delivered'}
              value={tileVal.weekToDate?.hatchingEggsDelivered}
            />
            ...
            <RenderLabelValue
              label={'Double Yolk Eggs'}
              value={tileVal.weekToDate?.doubleYolkEggs}
            />
            <RenderLabelValue
              label={'Salable Eggs'}
              value={
                tileVal.weekToDate?.doubleYolkEggs + tileVal.weekToDate?.rejectEggs ||
                'Unable to calculate'
              }
            />
            ...
            <RenderLabelValue
              ...
              value={tileVal.weekToDate?.totalEggsProduced}
            />
          </View>
          ...
          <View style={{marginBottom: 10}}>
            ...
            <RenderLabelValue
              label={'Eggs Delivered'}
              value={tileVal.totalToDate?.totalHatchingEggsDelivered}
            />
            <RenderLabelValue
              label={'Eggs Produced'}
              value={tileVal.totalToDate?.totalEggsProduced}
            />
            <RenderLabelValue
              label={'Hatching Eggs'}
              value={tileVal.totalToDate?.totalHatchingEggs}
            />
            <Text></Text>
            <RenderLabelValue
              label={'Double yolked Eggs'}
              value={tileVal.totalToDate?.totalDoubleYolkEggs}
            />
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {!loading ? (
        <View>
          <Text style={{color: '#282C50', fontSize: 24, fontWeight: 'bold'}}>
            RECORD EGG DELIVERY
          </Text>
          ...
          <View style={{height: 250, marginBottom: 20}}>
            ...
            <TouchableOpacity
              style={[
                styles.button,
                {flex: 0.7, marginTop: 20, marginLeft: 5, height: 'auto', justifyContent: 'center'},
              ]}
              onPress={() => {
                validateSubmission();
              }}>
