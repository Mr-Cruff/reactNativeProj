...
const EditFormDetails = ({route}) => {
  ...
  const saveForm = async () => {
    ...
    if (storedForms !== null) {
      storedFormsParsed.map(async (asyncForm, formIndex) => {
        if (storedFormsParsed[formIndex]['Form Id'] == updatedForm['Form Id']) {
          ...
          try {
            ...
            saveNSubmit
              ? SubmitForm(updatedForm).then(() => {
                  storedFormsParsed[formIndex] = {...storedFormsParsed[formIndex], ...updatedForm};
                  AsyncStorage.setItem('@forms', JSON.stringify(storedFormsParsed));
                  console.log(updatedForm['Status']);
                })
              : AsyncStorage.setItem('@forms', JSON.stringify(storedFormsParsed));
      ...
      if (!found) {
        ...
        console.log(x);
        await AsyncStorage.setItem('@forms', JSON.stringify(storedFormsParsed)).then(() =>
          saveNSubmit ? SubmitForm(updatedForm) : '',
        );
      }
  ...
  const SubmitForm = async form => {
    ...
    await axios
      .post(`https://devipbformdata.jabgl.com:84/api/FormDetails/submitFormDetails`, null, config)
      .then(response => {
  ...
  if (!loading) {
        ...
        <View style={styles.container}>
          ...
          {formFields.map((item, index) => {
            ...
            Object.keys(storedForm).map((cat, catIndex) => {
              if (
                cat == formFields[index].title &&
                typeof storedForm[cat] == 'object' &&
                Object.keys(storedForm[cat]).length != 0
              ) {
            ...
            return (
              <Category
                addToNewForm={addToNewForm}
                props={formFields[index]}
                key={index}
                asyncData={asyncData}
                save={save}
                setSave={setSave}
    ...
    return <LoadingScreen message="Loadingh, please wait ..." />;
  }
};
